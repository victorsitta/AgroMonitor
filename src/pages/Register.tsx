...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await signUpUser(form.email, form.password);
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#010a08]">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {isLoading ? <Loader2 className="animate-spin" /> : <>Criar Conta <ArrowRight size={18} /></>}
          </Button>

          <p className="text-center text-sm text-white/40">
            Já tem conta?{" "}
            <Link to="/login" className="text-emerald-400">Fazer login</Link>
          </p>
        </form>
      </motion.div>
    </div>
